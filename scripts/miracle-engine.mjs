...
// ═══════════════════════════════════════════════════════════════
// Usage:
//   node scripts/miracle-engine.mjs status        — 显示覆盖状态
//   node scripts/miracle-engine.mjs gap           — 列出缺失日期
//   node scripts/miracle-engine.mjs mark [date]   — 标记某日已覆盖
//   node scripts/miracle-engine.mjs init          — 初始化状态文件
//   node scripts/miracle-engine.mjs chapter [date]— 生成小说章节模板
//   node scripts/miracle-engine.mjs publish       — 准备发布包(含index.html)
//   node scripts/miracle-engine.mjs deploy        — 推送发布包到GitHub Pages
//   node scripts/miracle-engine.mjs novel-status  — 连载状态总览
...
const CHRONICLES_DIR = join(PROJECT, 'notes', 'chronicles');
const NOVEL_DIR = join(PROJECT, 'novel');
const PUBLISH_DIR = join(PROJECT, 'novel', 'publish');
...
async function main() {
  ...
  if (cmd === 'publish') {
    ...
    console.log(`  🌐 novel/publish/index.html         — GitHub Pages 首页`);
    console.log(`  📄 novel/publish/full-novel.md     — 全文合集 (${(Buffer.byteLength(fullNovel, 'utf-8') / 1024).toFixed(1)} KB)`);
    console.log(`  📋 novel/publish/chapter-index.md   — 章节目录`);
    console.log(`  📝 novel/publish/for-qidian.txt     — 起点/番茄格式`);
    console.log(`  📝 novel/publish/for-douban.md      — 豆瓣/知乎格式`);
    console.log(`\n  🚀 运行 node scripts/miracle-engine.mjs deploy 推送到 GitHub Pages`);
    process.exit(0);
  }
  ...
  if (cmd === 'deploy') {
    ...
    try {
      ...
      const cmds = [
        'git add novel/publish/ .github/workflows/deploy-pages.yml scripts/miracle-engine.mjs',
  ...
  console.log('');
  console.log('  状态与诊断:');
  console.log('    node scripts/miracle-engine.mjs status        显示覆盖状态');
  console.log('    node scripts/miracle-engine.mjs gap           列出缺失日期');
  console.log('    node scripts/miracle-engine.mjs novel-status  连载总览');
  console.log('');
  console.log('  创作工具:');
  console.log('    node scripts/miracle-engine.mjs chapter [date] 生成章节模板');
  console.log('    node scripts/miracle-engine.mjs mark [date]    标记日期已覆盖');
  console.log('');
  console.log('  发布工具:');
  console.log('    node scripts/miracle-engine.mjs publish        生成全平台发布包');
  console.log('    node scripts/miracle-engine.mjs deploy         推送到GitHub Pages');
  console.log('    node scripts/miracle-engine.mjs upload-guide   查看全平台投稿指南');
  console.log('');
  console.log('  系统:');
  console.log('    node scripts/miracle-engine.mjs init           初始化引擎');
  process.exit(0);
}
